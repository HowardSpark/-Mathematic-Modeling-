...
% sort(A)若A是向量不管是列还是行向量，默认都是对A进行生序排列。sort(A)是默认的升序，而sort(A,'descend')是降序排列。
% sort(A)若A是矩阵，默认对A的各列进行升序排列
% sort(A,dim)
% dim=1时等效于sort(A)
% dim=2时表示对A中的各行元素升序排列
% A = [2,1,3,8]
% Matlab中给一维向量排序使用的是sort函数：sort(A)，排序是按升序进行的，其中A为待排序的向量；
% 若想要保留排列前的索引，则可用[sA,index] =sort(A,'descend')，排序后，sA是排序好的向量，index是向量sA中对A的索引,结果如下：
% sA =
% 
%      8     3     2     1
% 
